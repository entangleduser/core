...
public extension KeyValueCollection {
 @inlinable func contains(_ key: Key) -> Bool { _elements[key] != nil }

 @inlinable func contains(_: Value) -> Bool {
...
public extension KeyValueCollection where Key == Int, Value: Hashable {
 @inlinable mutating func append(_ value: Value) {
  _elements[value.hashValue] = value
 }

 @inlinable
 mutating func append(_ values: [Value]) { values.forEach { self.append($0) } }
 ...
 init(arrayLiteral elements: Value...) { self.init(elements) }
}

#if os(macOS) || os(iOS) || os(watchOS) || os(tvOS)
 @available(macOS 10.15, iOS 13.0, watchOS 6.0, tvOS 13.0, *)
 public extension KeyValueCollection
 where Key == Value.ID, Value: Identifiable {
  @inlinable mutating func append(_ value: Value) {
